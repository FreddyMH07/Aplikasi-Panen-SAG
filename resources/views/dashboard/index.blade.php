...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
        ...
        <div class="flex flex-wrap gap-2">
            <span class="inline-flex items-center px-3 py-1 rounded-full bg-green-100 text-green-800 text-xs">ACV {{ number_format($todayMetrics['acv_prod'] ?? 0, 2) }}%</span>
            <span class="inline-flex items-center px-3 py-1 rounded-full bg-blue-100 text-blue-800 text-xs">AKP {{ number_format(($todayMetrics['akp'] ?? 0) * 100, 2) }}%</span>
            <span class="inline-flex items-center px-3 py-1 rounded-full bg-amber-100 text-amber-800 text-xs">BJR {{ number_format($todayMetrics['bjr'] ?? 0, 2) }}</span>
            <span class="inline-flex items-center px-3 py-1 rounded-full bg-rose-100 text-rose-800 text-xs">Restan {{ number_format($todayMetrics['restan_persen'] ?? 0, 2) }}%</span>
            <span class="inline-flex items-center px-3 py-1 rounded-full bg-purple-100 text-purple-800 text-xs">Refraksi {{ number_format($todayMetrics['refraksi_persen'] ?? 0, 2) }}%</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div class="p-4 bg-white dark:bg-gray-800 rounded-lg border">
            <div class="text-xs uppercase text-gray-500 dark:text-gray-400">ACV Produksi (Bulan)</div>
            <div class="mt-1 text-2xl font-semibold text-green-600">{{ number_format($monthlyMetrics['acv_prod'] ?? 0, 2) }}%</div>
        </div>
        <div class="p-4 bg-white dark:bg-gray-800 rounded-lg border">
            <div class="text-xs uppercase text-gray-500 dark:text-gray-400">AKP (Bulan)</div>
            <div class="mt-1 text-2xl font-semibold text-blue-600">{{ number_format(($monthlyMetrics['akp'] ?? 0) * 100, 2) }}%</div>
        </div>
        <div class="p-4 bg-white dark:bg-gray-800 rounded-lg border">
            <div class="text-xs uppercase text-gray-500 dark:text-gray-400">BJR (Bulan)</div>
